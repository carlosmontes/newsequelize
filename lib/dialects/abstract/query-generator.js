      ...
      for (key in valueHash) {
        if (valueHash.hasOwnProperty(key)) {
          ...
          if (modelAttributes && modelAttributes[key] && modelAttributes[key].autoIncrement === true && !value) {
            ...
            if (value && !!modelAttributes && !!modelAttributes[key] &&
                modelAttributes[key].type && modelAttributes[key].type.toString() === 'JSON' &&
                typeof value !== 'string') {
              try {
    ...
    */
    updateQuery: function(tableName, attrValueHash, where, options, attributes) {
      ...
      for (var key in attrValueHash) {
        ...
        if (value && !!attributes && !!attributes[key] &&
            attributes[key].type && attributes[key].type.toString() === 'JSON' &&
            typeof value !== 'string') {
          try {
            values.push(this.quoteIdentifier(key) + "=" + this.escape( JSON.stringify(value) ))
          } catch (e) {
            console.trace(e);
            console.log('JSON.stringify(value): ', value)
          }
      ...
      if (options.include) {
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (through) {
            ...
            // filter out the deleted rows on include table
            if (include.attributes.some(function(attr) {
              return attr == 'deletedAt'
            })) {
              joinQueryItem += self.quoteIdentifier(as) + '."deletedAt" IS NULL AND '
            }
